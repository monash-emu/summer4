...
current tag is `v0.2.0a2` (Diffrax `run()` JIT cache). The table is kept as
...
| WP12 | Pinnable release | `plans/wp12-release.plan.md` | **Applied.** First tag `v0.2.0a1`; current tag `v0.2.0a2`. JAX is a core dependency; `frames` declares polars and pyarrow. Downstream smoke CI is step 3 of {doc}`../dev/roadmap` |
| WP13 | Rate-tree math and tabular time series | `plans/tb-ports-feature-completeness.plan.md` | **Applied.** Math nodes, `TableInterp`, `Lookup`, `TraitChain.from_breakpoints` |
...
WP12 is **applied**: first tag `v0.2.0a1`, current tag `v0.2.0a2`. WP13 is
**applied**: math nodes (`Pow`, `Log`, `Maximum`, …), `TableInterp`, `Lookup`,
and `TraitChain.from_breakpoints`. Where the work has got to after that is
{doc}`../dev/roadmap`, not this page. WP14 adds a generalised force of infection
with a calibratable exponent and compartment-level infectiousness. WP15 adds
output algebra: `Trace` operators, a windowed `cumulative`, multi-flow
`FlowMass`, and named output sets to frames. WP16 benchmarks a TB-scale model
and makes solver failure visible. Plan:
